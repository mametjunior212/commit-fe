...
import { sumVotes, useLiveVotes } from '@/pages/RealtimeVoting';
import { Bar, BarChart, CartesianGrid, Cell, Legend, ResponsiveContainer, Tooltip, XAxis, YAxis } from 'recharts';
import { useDetailEvent } from '@/features/event/hooks/useDetailEvent';
...
const CaseStudyPage = () => {
  ...
  const nextProject = useMemo(() => apiEvent?.detailnextProject, [apiEvent, project?.detailnextProject]);
  const prevProject = useMemo(() => project?.detailprevProject, [apiEvent, project?.detailprevProject]);
  const isVoting = useMemo(() => project?.category === 'Voting', [project?.category]);
  ...
  const clean = (list: Items[]) =>
    list.filter((r) => (r?.value ?? 0) > 0).sort((a, b) => b.value - a.value);
  ...
  const totalUsers = dataVoting?.totaluser ?? 0;
  const votedUsers = useMemo(() => Math.min(totalVotes, totalUsers), [totalVotes, totalUsers]);
          ...
          <div className="border border-foreground/10 bg-background relative z-10">
            <div className="grid grid-cols-1 lg:grid-cols-4 border-b border-foreground/10">
              ...
              <div className="col-span-1 p-6 flex items-center justify-between lg:justify-center text-sm font-medium text-foreground/80">
                ...
                <div className="flex items-center gap-2 font-mono">{project?.year}</div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12">
              <div className="lg:col-span-12 p-6 md:p-12 lg:p-16 border-b border-foreground/10">
                ...
                <div className="mt-8 md:mt-12 flex flex-col md:flex-row md:items-center justify-between gap-6">
                  <p className="text-lg md:text-xl text-foreground/60 max-w-4xl leading-relaxed">{project?.description}</p>
                ...
                >
                  {project?.about && project?.about !== '' && (
                    <>
                      ...
                      <p className="mb-12 text-foreground/80 leading-relaxed" style={{ whiteSpace: 'pre-line' }}>
                        {project?.about}
                      </p>
                    </>
                  )}
                  ...
                  <motion.div>
                    ...
                    <motion.div className="grid grid-cols-2 md:grid-cols-5 gap-3 mt-10">
                      {project?.partner.map((e: { img: string; partner: string; link: string }, index: number) => (
                        <motion.div key={index} className="rounded-xl border-[#f1f0f8] relative min-w-2 border-[3px]">
                          <motion.img
                            key={index}
                  ...
                  <motion.div>
                    ...
                    <motion.div className="grid grid-cols-2 md:grid-cols-5 gap-3 mt-10">
                      {project?.media.map((e: { img: string; media: string; link: string }, index: number) => (
                        <motion.a key={index} href={e.link} className="rounded-xl border-[#f1f0f8] relative min-w-2 border-[3px]">
                          <motion.img
                            key={index}
                            src={import.meta.env.VITE_FONT_END + e.img}
                            alt={e.media}
                      ...
                      <div className="relative overflow-hidden rounded-2xl border bg-card p-4">
                        <div className="mb-3 flex items-center justify-between">
                          <div>
                            ...
                            <div className="text-2xl font-semibold tabular-nums">{totalVotes.toLocaleString('id-ID')}</div>
                          </div>
                          <div className="text-right text-xs text-muted-foreground">
                            Terakhir diperbarui
                            <br />
                            <span className="font-medium">{format(lastUpdated, 'dd MMM yyyy HH:mm:ss', { locale: localeID })}</span>
                          </div>
                        </div>
                        ...
                        <div className="h-72">
                          <ResponsiveContainer width="100%" height="100%">
                            <BarChart data={rows} margin={{ top: 8, right: 16, bottom: 8, left: 0 }}>
                              ...
                              <Tooltip formatter={(value: any) => [Number(value).toLocaleString('id-ID'), 'Suara']} />
                              <Legend />
                            ...
                            <div key={it.candidateId} className="rounded-xl border p-3">
                              ...
                              <Progress.Root className="relative h-2 w-full overflow-hidden rounded-full bg-muted">
                                <Progress.Indicator
                                  ...
                                  style={{ transform: `translateX(-${100 - it.percent}%)`, backgroundColor: it.color }}
                                />
                              </Progress.Root>
                            </div>
                          ))}
        ...
        {project?.nextProject && project?.nextProject !== '' && (
          <section className="border-t border-foreground/10 bg-foreground/5 py-20">
            <div className="container-wide max-w-[90rem] mx-auto px-4 sm:px-6">
              ...
              {nextProject ? (
                <Link to={`/event/${nextProject.uuid}`} className="group block border border-foreground/10 bg-background p-8 hover:border-accent transition-colors relative overflow-hidden">
                  <div className="grid md:grid-cols-2 gap-8 items-center">
                    <div className="aspect-[16/9] overflow-hidden bg-foreground/5">
                      <img src={import.meta.env.VITE_FONT_END + nextProject.heroImage} alt={nextProject.title} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105" />
                    </div>
                    <div>
                ...
                <div className="text-center py-12 text-foreground/40 italic">End of portfolio.</div>
              )}
            </div>
          </section>
        )}
        ...
        {project?.prevProject && project?.prevProject !== '' && (
          <section className="border-t border-foreground/10 bg-foreground/5 py-20">
            <div className="container-wide max-w-[90rem] mx-auto px-4 sm:px-6">
              ...
              {prevProject ? (
                <Link to={`/event/${prevProject.uuid}`} className="group block border border-foreground/10 bg-background p-8 hover:border-accent transition-colors relative overflow-hidden">
                  <div className="grid md:grid-cols-2 gap-8 items-center">
                    <div className="aspect-[16/9] overflow-hidden bg-foreground/5">
                      <img src={import.meta.env.VITE_FONT_END + prevProject.heroImage} alt={prevProject.title} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105" />
                    </div>
                    <div>
                      <div className="flex items-center gap-3 text-xs font-bold uppercase tracking-widest text-foreground/40 mb-4">
