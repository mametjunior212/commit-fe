...
export function DayCell({ cell, events, eventPositions }: IProps) {
  ...
  const renderEventAtPosition = useCallback(
    (position: number) => {
      ...
      return (
        ...
        >
          {showBullet && (
            <EventBullet className="lg:hidden" color={toTEventColor(event.color)} />
          )}
          <MonthEventBadge
  ...
  const cellContent = useMemo(
    () => (
      ...
      <motion.div
        className={cn(
          "flex h-full lg:min-h-30 flex-col gap-1 border-l border-andrika border-t",
      ...
      >
        <DroppableArea date={date} className="w-full h-full py-2">
          <motion.span
            className={cn(
              "h-6 px-1 text-xs font-semibold text-andrika lg:px-2",
          ...
          <motion.div
            className={cn(
              "flex h-fit gap-1 px-2 mt-1 lg:h-[64px] lg:flex-col lg:gap-2 lg:px-0",
          ...
          >
            {
              ...
              //       <Button
              //         variant="ghost"
              //         className="border border-andrika opacity-0 group-hover:opacity-100 transition-opacity duration-200"
              //       >
